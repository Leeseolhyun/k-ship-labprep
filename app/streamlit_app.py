...
탭 구성
  1. 당일 근태     : 사원증 태깅·가용 인력 풀 확인
  2. 데이터 개요   : 블록·작업자 현황
  3. 공수 예측     : 병목 후보 + SHAP 근거
  4. 스케줄 최적화 : GA 실행 → 간트차트 / 부하율 / 수렴곡선
  5. What-If      : 작업자 수·인원 한도 조절 후 재최적화
  6. 준법 점검     : 법령·선급규칙 위반 여부
"""
...
def load_raw():
    cfg = load_config()
    paths = cfg["paths"]
    missing = [k for k in ("blocks_csv", "workers_csv", "jobs_csv", "precedence_csv", "attendance_csv")
    ...
    return cfg, (
        ...
        pd.read_csv(paths["attendance_csv"]),
    )
...
blocks_all, workers_all, jobs_all, prec, attendance_seed = raw
bundle = load_model(cfg)
if bundle is None:
    st.error("학습된 모델이 없습니다. `python scripts/02_train_model.py` 를 먼저 실행하세요.")
    st.stop()

# 근태 데이터는 GA 실행마다 달라지는 핵심 입력이다. 세션 상태에 보관하므로
# 데모의 '사원증 태깅' 버튼을 누르면 바로 가용 인력과 스케줄 대상이 갱신된다.
if "attendance_snapshot" not in st.session_state:
    st.session_state["attendance_snapshot"] = normalize_attendance(attendance_seed)
attendance = normalize_attendance(st.session_state["attendance_snapshot"])
available_workers_all = available_workers(workers_all, attendance)
if available_workers_all.empty:
    st.error("당일 가용 작업자가 없습니다. 근태 탭에서 출근 태깅을 먼저 처리하세요.")
    st.stop()
...
with st.sidebar:
    st.header("시나리오 설정")
    st.success(f"당일 가용 인력 {len(available_workers_all)} / {len(workers_all)}명")
    st.caption("근태 상태가 바뀌면 가용 인력 풀이 자동으로 갱신됩니다.")
    n_blocks = st.slider("스케줄 대상 블록 수", 6, min(60, len(blocks_all)),
                         cfg["scheduling"]["n_blocks_to_schedule"], step=2)
    min_workers = min(10, len(available_workers_all))
    n_workers = st.slider("최적화에 투입할 가용 작업자 수", min_workers,
                          len(available_workers_all), len(available_workers_all), step=1)
...
# 근태 스냅샷에서 출근·지각으로 판정된 인력만 GA에 전달한다.
workers = available_workers_all.head(n_workers)
...
attendance_tab, overview_tab, prediction_tab, schedule_tab, what_if_tab, compliance_tab = st.tabs([
    "당일 근태", "데이터 개요", "공수 예측", "스케줄 최적화", "What-If 분석", "준법 점검"
])


# ---------------------------------------------------------------------
# 1. 당일 근태: 사내 API 연동의 프로토타입 화면
# ---------------------------------------------------------------------
with attendance_tab:
    st.subheader("당일 가용 인력 — 근태 시스템 연동 프로토타입")
    st.caption("운영 환경에서는 출입 카드·ERP·HR API가 이 스냅샷을 갱신합니다. 현재는 사원증 태깅을 가상으로 시연합니다.")
    c = st.columns(4)
    c[0].metric("등록 작업자", f"{len(workers_all)}명")
    c[1].metric("당일 가용", f"{len(available_workers_all)}명")
    c[2].metric("미가용", f"{len(workers_all) - len(available_workers_all)}명")
    c[3].metric("유자격 용접사", f"{int(available_workers_all['certified_welder'].sum())}명")

    status_view = workers_all.merge(
        attendance[["worker_id", "status", "check_in_time", "available_from_min", "source", "is_available"]],
        on="worker_id", how="left",
    )
    st.dataframe(status_view[["worker_id", "skill", "career_years", "certified_welder", "status", "check_in_time", "is_available", "source"]],
                 use_container_width=True, height=320)

    unavailable = status_view.loc[~status_view["is_available"], "worker_id"].tolist()
    if unavailable:
        selected_worker = st.selectbox("사원증 태깅 시뮬레이션 — 출근 처리할 작업자", unavailable)
        if st.button("사원증 태깅 후 가용 인력 풀 갱신", type="primary"):
            st.session_state["attendance_snapshot"] = check_in_worker(attendance, selected_worker)
            # 기존 결과는 이전 인력 풀로 계산됐으므로 혼동을 막기 위해 폐기한다.
            st.session_state.pop("result", None)
            st.session_state.pop("base_score", None)
            st.success(f"{selected_worker} 출근 처리 완료 — 다음 최적화부터 반영됩니다.")
            st.rerun()
    else:
        st.success("모든 등록 작업자가 현재 가용 상태입니다.")
...
with what_if_tab:
    ...
    available_count = len(available_workers_all)
    ...
    if st.button("What-If 시뮬레이션 실행"):
        ...
        for i, w in enumerate(sorted(set(candidates))):
            # What-If 역시 근태상 가용 인력 안에서만 규모를 바꾼다.
            wk = available_workers_all.head(w)
